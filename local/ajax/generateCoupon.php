...
$fields['TITLE'] = 'Запрос промокода';
$fields['URL'] = $_SERVER['SERVER_NAME'].$_POST['url'];
$string = $_POST['url'];
if(preg_match('/\/([0-9]+)-/',$string,$matches)){
    $fields['ARTICLE'] = $matches[1];
}

\Bitrix\Main\Loader::IncludeModule('sale');
$arCoupon = \Bitrix\Sale\Internals\DiscountCouponTable::getRow([
    'filter' => [
        'ID' => \Bitrix\Main\Config\Option::get( "askaron.settings", "UF_COUPON_ID"),
    ]
]);
...
if (!empty($arCoupon))
{
    ...
    $r = send_sms_mail($phone,'Ваш купон: ' . $arCoupon['COUPON']);
    if ($r > 0){
        $fields['PROMOCODE'] = $arCoupon['COUPON'];
        $sale = CSaleDiscount::GetByID($arCoupon['DISCOUNT_ID']);
        $fields['PROMOCODE_SALE'] = $sale['NAME'];
        $fields['PROMOCODE_SALE'] = preg_replace('/[^0-9]/', '', $fields['PROMOCODE_SALE']);
        ...
        if($result['total'] == 0) {
            ...
            $queryData = http_build_query(array(
                ...
                'fields' => [
                    'NAME' => $fields['CONTACT_NAME'] ? $fields['CONTACT_NAME'] : $phone,
                    'SECOND_NAME' => $fields['CONTACT_NAME'] ? $fields['CONTACT_NAME'] : $phone,
                    'LAST_NAME' => $fields['CONTACT_NAME'] ? $fields['CONTACT_NAME'] : $phone,
                    'PHONE' => [['VALUE' => $phone,"VALUE_TYPE" => "WORK" ]],
                ],
                'params' => [
        ...
        $queryData = http_build_query(array(
            'fields' => array(
                ...
                'UF_CRM_1652724485589' => intval($fields['PROMOCODE_SALE']),
                'UF_CRM_1652724624430' => $fields['URL'],
                'UF_CRM_1652724655987' => $fields['ARTICLE'],
            ),
