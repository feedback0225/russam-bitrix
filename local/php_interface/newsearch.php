...
class CNewSearch extends CDBResult
{
	...
	function Search($arParams, $aSort = array(), $aParamsEx = array(), $bTagsCloud = false)
	{
		...
		else
		{
			...
			if (is_array($aParamsEx) && !empty($aParamsEx))
			{
				...
				{
					$strSqlWhere = CNewSearch::__PrepareFilter($aParamEx, $bIncSites);
					if ($strSqlWhere != "")
			...
			$strSqlWhere = CNewSearch::__PrepareFilter($arParams, $bIncSites);
			if ($strSqlWhere != "")
			...
			$csearch = new CSearch();
			if ($bTagsCloud)
				$strSql = $csearch->tagsMakeSQL($query, $this->strSqlWhere, $strSqlOrder, $bIncSites, $bStem, $aParamsEx["LIMIT"]);
			else
				$strSql = $csearch->MakeSQL($query, $this->strSqlWhere, $strSqlOrder, $bIncSites, $bStem);
	...
	public static function __PrepareFilter($arFilter, &$bIncSites, $strSearchContentAlias = "sc.")
	{
		...
		$obQueryWhere->SetFields(array(
			...
			"PARAM3" => array(
				"TABLE_ALIAS" => $strSearchContentAlias,
				"FIELD_NAME" => $strSearchContentAlias.".PARAM3",
				"MULTIPLE" => "N",
				"FIELD_TYPE" => "string",
				"JOIN" => false,
			),
