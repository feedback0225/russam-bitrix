		...
		foreach ($products as $items) {
			foreach ($items as $product) {
				...
				while ($row = $result->Fetch()) {
					...
					while ($row2 = $result2->Fetch()) {
						...
						if (isset($quantity['QUANTITY']) && !empty($price['PRICE']) && $active['ACTIVE'] == 'Y') {
							...
							$result2Rows[$row2['IBLOCK_ELEMENT_ID']]['price'] = isset($price['PRICE']) ? (int)$price['PRICE'] : 0;
							$result2Rows[$row2['IBLOCK_ELEMENT_ID']]['quantity'] =  (int)$quantity['QUANTITY'];
						}
						if(!$quantity['QUANTITY']){
							$result2Rows[$row2['IBLOCK_ELEMENT_ID']]['quantity'] = (int)CCatalogProduct::GetByID($row2['IBLOCK_ELEMENT_ID'])['QUANTITY'];
				...
				if (empty($result2Rows)) {
					...
					$result2Rows['fa']['price'] = isset($price['PRICE']) ? (int)$price['PRICE'] : 0;
					$result2Rows['fa']['quantity'] =  (int)$quantity['QUANTITY'];
				...
				if (!empty($result2Rows)) {
							...
							// if ($article2 == $article1) {
								if($products_arrs[$article2]['price'] < $result2Row['price']):
									...
									if(isset($products_arrs[$article2])){
										if($products_arrs[$article2]['quantity']){
					...
					if (empty($unset) && !$products_arrs[$article]) {
				...
				// if($product['product_id'] == 494889425){
				// 	global $APPLICATION;
				// 	$APPLICATION->RestartBuffer();
				// 	echo '<pre>';
				// 	var_dump($article);
				// 	var_dump($result2Rows);
				// 	die();
				// }
