...
<section class="collections g-filters">
	<div class="container collections__container">
		...
		<h1 class="g-title g-title--center catalog__title search-results__title">
			...
			if ($_GET['search']) :
				...
				while ($arSearch = $obSearch->Fetch()) {
					...
					//similar_text($arSearch['TITLE'], $_REQUEST['search'], $arSearch['TITLERANK']);
					$arItems[$arSearch['ITEM_ID']] = $arSearch['ITEM_ID'];
				}
				if (count($arItems) == 0) {
			...
			?>
			<span>Результаты поиска <?= $_REQUEST['search'] ? 'по запросу “' . htmlspecialchars($_REQUEST['search']) . '”' : '' ?></span>
		...
		<div class="catalog__content">
			<?
			if ($_REQUEST['vstavki'] == 'off') {
				$smartPreFilter['PROPERTY_VSTAVKA'] = false;
			}
			if ($_REQUEST['noname'] == 'y') {
