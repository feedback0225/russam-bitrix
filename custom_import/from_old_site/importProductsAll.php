...
?>
    <div class="container">
        ...
        $workbook = $reader->load("exportProductsNew.xlsx");
        ...
        foreach ($workBook as $key=>$dt) {
            ...
            foreach (json_decode($dt[14])->templates as $key1=>$vstavka){ //Шаблон_вставки
                $vstavkaStr = mb_ucfirst($vstavka->каменьШаблонаВставки) . ' - ' . $vstavka->количествоШаблонаВставок . ' ,огранка «' . $vstavka->формаОгранкиШаблонаВставки . '»';
                if($vstavka->группаЦветаШаблонаВстваки){
                    $vstavkaStr .= ', цвет ' . $vstavka->группаЦветаШаблонаВстваки;
                }
                if($vstavka->группаЧистотыШаблонаВстваки){
                    $vstavkaStr .= ', чистота ' . $vstavka->группаЧистотыШаблонаВстваки;
                }
                $vstavkaStr .= ', ' . $vstavka->весШаблонаВставки*$vstavka->количествоШаблонаВставок . ' crt';
                $PROP['VSTAVKA'][] = $vstavkaStr;
                $PROP['VSTAVKA_COLOR'][] = $vstavka->цветШаблонаВставкии;
                if($vstavka->каменьШаблонаВставки){
                    $entity_data_class = GetEntityDataClass(18);
                    $curHLBTName = mb_ucfirst($vstavka->каменьШаблонаВставки);
                    ...
                    while($el = $rsData->fetch()){
                        $PROP['VSTAVKA_FILTER']['n'.$nCount]['VALUE'] = $el['UF_XML_ID'];
                        $exist = true;
                    }
                    if(!$exist){
                        ...
                        $PROP['VSTAVKA_FILTER']['n'.$nCount]['VALUE'] = 'id-'.Cutil::translit($curHLBTName,"ru",array("replace_space"=>"-","replace_other"=>"-"));
                    }
                    $nCount++;
                }
            }
            ...
            if($productArrayInitial['Вес_изделия'] && $productArrayInitial['Вес_изделия'] != 'null'){
                $PROP['PRODUCT_PRODUCT_WEIGHT'] = $productArrayInitial['Вес_изделия'];
            }
            ...
            if($productArrayInitial['Ширина'] && $productArrayInitial['Ширина'] != 'null'){
                $PROP['PRODUCT_WIDTH'] = $productArrayInitial['Ширина'];
            }
            ...
            if($productArrayInitial['Причина_сокрытия_товара']){
                $PROP['WHY_HIDE'] = $productArrayInitial['Причина_сокрытия_товара'];
            }
